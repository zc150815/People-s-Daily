...
-(void)loadData{
    ...
    self.dataArr = @[@{@"titleStr":@"Clear Cache"},
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([titleStr containsString:@"Clear Cache"]) {
        [self cleanSystemCache];
        [self.tableView reloadData];
        [[PDPublicTools sharedPublicTools]showMessage:@"清除缓存完成" duration:3];
    }else if ([titleStr containsString:@"Terms of Service"]){
        PDWebViewController *webviewVC = [[PDWebViewController alloc]init];
        webviewVC.title = titleStr;
        webviewVC.dataType = PDWebViewDataTypeTermsOfService;
        [self.navigationController pushViewController:webviewVC animated:YES];
    }else if ([titleStr containsString:@"Privacy Policy"]){
        PDWebViewController *webviewVC = [[PDWebViewController alloc]init];
        webviewVC.title = titleStr;
        webviewVC.dataType = PDWebViewDataTypePrivacyPolicy;
        [self.navigationController pushViewController:webviewVC animated:YES];
    }
}
